...
import (
	...
	"github.com/LCY2013/paas/route/domain/repository"
	"path/filepath"

	ds "github.com/LCY2013/paas/route/domain/service"
	"github.com/LCY2013/paas/route/handler"
	//"github.com/afex/hystrix-go/hystrix"
	"github.com/LCY2013/paas/route/proto/route"
...
func main() {
	...
	//4.添加链路追踪
	t, io, err := trace.NewTracer("go.micro.service.route", tracerHost+":"+strconv.Itoa(tracerPort))
	if err != nil {
		log.Error(err)
	}
	defer io.Close()
	...
	// 注册句柄，可以快速操作已开发的服务
	routeDataService := ds.NewRouteDataService(repository.NewRouteRepository(db), clientset)
	err = route.RegisterRouteHandler(service.Server(), &handler.RouteHandler{RouteDataService: routeDataService})
	if err != nil {
		log.Fatal(err)
		return
	}
