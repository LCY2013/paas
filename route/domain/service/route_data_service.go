...
func (u *RouteDataService) CreateRouteToK8s(info *route.RouteInfo) (err error) {
	...
	if _, err = u.K8sClientSet.
		NetworkingV1().
		Ingresses(info.RouteNamespace).
		Get(context.TODO(), info.RouteName, v14.GetOptions{}); err != nil {
		if _, err = u.K8sClientSet.
			NetworkingV1().
			Ingresses(info.RouteNamespace).
			Create(context.TODO(), ingress, v14.CreateOptions{}); err != nil {
			//创建不成功记录错误
			log.Error(err)
			return err
		}
		...
		log.Error("路由 " + info.RouteName + " 已经存在")
...
// getIngressPath 根据info信息获取path路径
...
func (u *RouteDataService) DeleteRouteFromK8s(route2 *model.Route) (err error) {
	...
	if err = u.K8sClientSet.NetworkingV1().Ingresses(route2.RouteNamespace).Delete(context.TODO(), route2.RouteName, v14.DeleteOptions{}); err != nil {
		...
		log.Info("删除 ingress ID：" + strconv.FormatInt(route2.ID, 10) + " 成功！")
	}
	return
}
