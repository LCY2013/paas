...
func (d *DNSProvider) Present(domain, token, keyAuth string) error {
	...
	dbsRecord := cloudflare.CreateDNSRecordParams{
		Type:    "TXT",
		Name:    dns01.UnFqdn(fqdn),
		Content: value,
		TTL:     d.config.TTL,
	}
	response, err := d.client.clientEdit.CreateDNSRecord(context.TODO(), cloudflare.ResourceIdentifier(zoneID), dbsRecord)
	if err != nil {
...
func (d *DNSProvider) CleanUp(domain, token, keyAuth string) error {
	...
	err = d.client.clientEdit.DeleteDNSRecord(context.TODO(), cloudflare.ResourceIdentifier(zoneID), recordID)
	if err != nil {
