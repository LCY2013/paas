...
import (
	...
	go_micro_service_permission "github.com/LCY2013/paas/user/proto/permission"
	go_micro_service_role "github.com/LCY2013/paas/user/proto/role"
	go_micro_service_user "github.com/LCY2013/paas/user/proto/user"
	...
	"github.com/LCY2013/paas/userapi/handler"
	hystrix2 "github.com/LCY2013/paas/userapi/plugin/hystrix"
...
func main() {
	...
	//2.添加链路追踪
	t, io, err := trace.NewTracer("go.micro.api.userapi", tracerHost+":"+strconv.Itoa(tracerPort))
	if err != nil {
		log.Error(err)
	}
	defer io.Close()
	...
	roleService := go_micro_service_role.NewRoleService("go.micro.service.user", service.Client())
	permissionService := go_micro_service_permission.NewPermissionService("go.micro.service.user", service.Client())
	// 注册控制器
	if err = userapi.RegisterUserapiHandler(service.Server(), &handler.Userapi{
