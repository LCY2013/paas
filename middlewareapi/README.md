...
# 1.快速创建代码仓库请使用下方命令
```
sudo docker run --rm -v $(pwd): $(pwd) -w  $(pwd) luochunyun/tool newService github.com/xxx/middlewareapi

注意：
1.sudo 如果是 Mac 系统提示输入的密码是你本机的密码。
2.这里github.com/xxx/middlewareapi 仓库名字需要和 go mod 名称一致
```
...
make build
```
代码执行后会产生 middlewareapi 二进制文件

## 4.编译执行二进制文件
```
make docker
```
编译成功后会自动生成 middlewareapi:latest 镜像
可使用 docker images |grep middlewareapi 查看是否生成
