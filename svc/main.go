...
import (
	...
	ds "github.com/LCY2013/paas/svc/domain/service"
	"github.com/LCY2013/paas/svc/handler"
...
func main() {
	...
	//4.添加链路追踪
	t, io, err := trace.NewTracer("go.micro.service.svc", tracerHost+":"+strconv.Itoa(tracerPort))
	if err != nil {
		log.Error(err)
	}
	defer io.Close()
	...
	// 注册句柄，可以快速操作已开发的服务
	svcDataService := ds.NewSvcDataService(repository.NewSvcRepository(db), clientset)
