...
import (
	...
	ds "github.com/LCY2013/paas/volume/domain/service"
	"github.com/LCY2013/paas/volume/handler"
...
func main() {
	...
	//4.添加链路追踪
	t, io, err := trace.NewTracer("go.micro.service.volume", tracerHost+":"+strconv.Itoa(tracerPort))
	if err != nil {
		log.Error(err)
	}
	defer io.Close()
	...
	// 注册句柄，可以快速操作已开发的服务
	volumeDataService := ds.NewVolumeDataService(repository.NewVolumeRepository(db), clientset)
