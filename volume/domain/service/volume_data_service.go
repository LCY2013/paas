...
import (
	...
	"github.com/LCY2013/paas/volume/domain/repository"
	"github.com/LCY2013/paas/volume/proto/volume"
	"k8s.io/api/apps/v1"
...
// DeleteVolumeFromK8s 从 k8s 删除一个 pvc
func (u *VolumeDataService) DeleteVolumeFromK8s(volume *model.Volume) (err error) {
	//先从K8s 中删除
	if err = u.K8sClientSet.
		CoreV1().
		PersistentVolumeClaims(volume.VolumeNamespace).
		Delete(context.TODO(), volume.VolumeName, v13.DeleteOptions{}); err != nil {
		log.Error(err)
		return err
	} else {
		//从数据表中删除
		...
		log.Info("删除存储ID" + strconv.FormatInt(volume.ID, 10) + " 成功！")
	}
	return
}

// CreateVolumeToK8s 创建存储到 k8s 中
func (u *VolumeDataService) CreateVolumeToK8s(info *volume.VolumeInfo) (err error) {
	...
	if _, err = u.K8sClientSet.
		CoreV1().
		PersistentVolumeClaims(info.VolumeNamespace).
		Get(context.TODO(), info.VolumeName, v13.GetOptions{}); err != nil {
		//如果存储不存在
		if _, err = u.K8sClientSet.
			CoreV1().
			PersistentVolumeClaims(info.VolumeNamespace).
			Create(context.TODO(), volume, v13.CreateOptions{}); err != nil {
			log.Error(err)
			return err
		}
		log.Info("存储创建成功")
		...
		log.Error("存储空间" + info.VolumeName + " 已经存在")
...
func (u *VolumeDataService) getResource(info *volume.VolumeInfo) (source v12.ResourceRequirements) {
	source.Requests = v12.ResourceList{
		v12.ResourceStorage: resource.MustParse(strconv.FormatFloat(float64(info.VolumeRequest), 'f', 6, 64) + "Gi"),
	}
	return
}
