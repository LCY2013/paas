...
import (
	...
	ds "github.com/LCY2013/paas/appstore/domain/service"
	"github.com/LCY2013/paas/appstore/handler"
	//"github.com/afex/hystrix-go/hystrix"
	"github.com/LCY2013/paas/appstore/proto/appstore"
...
func main() {
	...
	//4.添加链路追踪
	t, io, err := trace.NewTracer("go.micro.service.appstore", tracerHost+":"+strconv.Itoa(tracerPort))
	if err != nil {
		log.Error(err)
	}
	defer io.Close()
	...
	// 注册句柄，可以快速操作已开发的服务
	appstoreDataService := ds.NewAppstoreDataService(repository.NewAppstoreRepository(db), clientset)
	err = appstore.RegisterAppstoreHandler(service.Server(), &handler.AppstoreHandler{AppstoreDataService: appstoreDataService})
	if err != nil {
		log.Fatal(err)
		return
	}
