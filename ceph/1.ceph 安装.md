### 慕课网 Cap 基于 Centos8 系统通过 Cephadm 快速部署  Ceph16（pacific）版本
...
192.168.0.180 ceph01  2Cpu 4G内存  20G系统盘，20G数据盘
192.168.0.181 ceph02  2Cpu 4G内存  20G系统盘，20G数据盘
192.168.0.182 ceph03  2Cpu 4G内存  20G系统盘，20G数据盘
```shell
cat >> /etc/hosts <<EOF
192.168.0.180 ceph01 
192.168.0.181 ceph02 
192.168.0.182 ceph03 
EOF
```
...
```shell
cephadm bootstrap --mon-ip 192.168.0.180
    
cephadm bootstrap --mon-ip 192.168.0.180 --cluster-network 192.168.0.0/24 --allow-fqdn-hostname
```
此命令将会进行以下操作：
...
安装日志如下为成功
```shell
[root@ceph01 ~]# cephadm bootstrap --mon-ip 192.168.0.180
...
Verifying IP 192.168.0.181 port 3300 ...
...
Unit chronyd.service is enabled and running
Host looks OK
Cluster fsid: 756da094-baac-11ed-b1c8-05cc32ab3be6
Verifying IP 192.168.0.180 port 3300 ...
Verifying IP 192.168.0.180 port 6789 ...
Mon IP `192.168.0.180` is in CIDR network `172.31.96.0/20`
...
You can access the Ceph CLI as following in case of multi-cluster or non-default config:
        ...
        sudo /usr/sbin/cephadm shell --fsid 756da094-baac-11ed-b1c8-05cc32ab3be6 -c /etc/ceph/ceph.conf -k /etc/ceph/ceph.client.admin.keyring
...
```shell
[root@ceph01 ~]# cephadm bootstrap --mon-ip 192.168.0.180 --cluster-network 192.168.0.0/24 --allow-fqdn-hostname
Verifying podman|docker is present...
Verifying lvm2 is present...
Verifying time synchronization is in place...
...
Verifying IP 192.168.0.180 port 3300 ...
Verifying IP 192.168.0.180 port 6789 ...
Mon IP `192.168.0.180` is in CIDR network `192.168.0.0/24`
Mon IP `192.168.0.180` is in CIDR network `192.168.0.0/24`
...
Setting cluster_network to 192.168.0.0/24
Wrote config to /etc/ceph/ceph.conf
Wrote keyring to /etc/ceph/ceph.client.admin.keyring
Creating mgr...
Verifying port 9283 ...
...
Ceph Dashboard is now available at:
	    ...
	    User: admin
	Password: 0z1xitcvnw
	new-Password: 0z1xitcvnw321

Enabling client.admin keyring and conf on hosts with "admin" label
Saving cluster configuration to /var/lib/ceph/756da094-baac-11ed-b1c8-05cc32ab3be6/config directory
Enabling autotune for osd_memory_target
You can access the Ceph CLI as following in case of multi-cluster or non-default config:
...
添加ceph 02 节点
```shell
ceph orch host add ceph02 192.168.0.181
```
添加ceph 03 节点
```shell
ceph orch host add ceph03 192.168.0.182
```
...
ceph01  192.168.0.180  _admin          
ceph02  192.168.0.181                  
ceph03  192.168.0.182                  
3 hosts in cluster
```
...
[root@ceph01 ~]# ceph -s
  cluster:
    id:     756da094-baac-11ed-b1c8-05cc32ab3be6
    health: HEALTH_OK
...
spec:
  pool: iscsi_pool
  trusted_ip_list: "192.168.0.180,192.168.0.181,192.168.0.182"
  api_user: admin
  api_password: admin
  api_secure: false
